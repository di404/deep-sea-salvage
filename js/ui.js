...
function refreshBuffs() {
  ...
  for (const b of activeBuffs(st)) {
    ...
    const chip = el(`<span class="buff-chip">${def.em} ${fmtTimeShort(left)}</span>`);
    box.appendChild(chip);
  }
  if (chestDue()) {
    const btn = el(`<button class="buff-chip" style="border-color:var(--gold);color:var(--gold)">🎁 ${L('openChest')}</button>`);
...
export function closePanel() { panelOpen = null; $('#panel').classList.add('hidden'); }
...
function renderUpgrades(body) {
  ...
  if (nextZ) {
    ...
    const row = el(`<div class="row" style="border-color:var(--gold)">
      ...
      <button class="btn gold ${can ? '' : 'gray'}" ${can ? '' : 'disabled'}>🪙 ${fmt(nextZ.cost)}</button></div>`);
  ...
  for (const u of list) {
    ...
    const row = el(`<div class="row up-row ${maxed ? 'maxed' : ''}">
      ...
      <button class="btn ${can ? 'gold' : 'gray'}" ${can ? '' : 'disabled'}>${maxed ? L('maxed') : '🪙 ' + fmt(cost)}</button></div>`);
  ...
  if (st.zone >= 3) {
    ...
    if (rs.active) {
      ...
      const row = el(`<div class="row"><div class="grow"><h4>🔬 ${L('research')}</h4><div class="sub">${zh ? '开始一项限时研究获得增益' : 'Start a timed research for buffs'}</div></div><button class="btn">${L('contractGo')}</button></div>`);
...
function renderBook(body) {
  ...
  for (const cdef of list) {
    ...
    const slot = el(`<div class="slot ${got ? 'got' : 'locked'} ${cdef.r >= 2 ? 'r' + cdef.r : ''}">
      <span class="rd">${cdef.r > 0 ? '★'.repeat(cdef.r) : ''}</span></div>`);
    ...
    slot.appendChild(el(`<div class="val">${got ? '🪙' + fmt(cdef.v) + ' ×' + st.book[cdef.id] : '???'}</div>`));
...
function renderShop(body) {
  ...
  const hero = el(`<div class="shop-hero">
    ...
    <div style="display:flex;gap:8px;justify-content:center;margin-top:10px;flex-wrap:wrap">
      <button class="btn gem ad" id="sh-gems">${L('adGems')}${adUsesLeft('free_gems') !== Infinity ? ' (' + adUsesLeft('free_gems') + ')' : ''}</button>
      <button class="btn green ad" id="sh-buff">⚡ 2× 4h ${adUsesLeft('buff_2x') !== Infinity ? '(' + adUsesLeft('buff_2x') + ')' : ''}</button>
    </div></div>`);
  ...
  hero.querySelector('#sh-buff').onclick = async () => {
    if (await requestAd('buff_2x')) {
      meta.addBuff('x2', 240);
      SFX.upgrade(); floatToast('⚡ 2× 4h');
      renderPanel(); refreshDots(); writeSave();
    }
  };
  body.appendChild(hero);
  // wheel + gacha entries
  const row = el(`<div class="row"><div class="grow"><h4>🎡 ${L('wheel')}</h4><div class="sub">${L('freeSpins')}: ${st.wheel.free}</div></div><button class="btn gold">${L('spin')}</button></div>`);
  row.querySelector('button').onclick = () => openWheel();
  body.appendChild(row);
  const g = meta.gachaState();
  const row2 = el(`<div class="row"><div class="grow"><h4>🥚 ${L('gacha')}</h4><div class="sub">${L('pityNote')} · 🎟️×${g.tickets}</div></div><button class="btn gem">${L('open1')}</button></div>`);
  ...
  for (const s of SKINS) {
    ...
    if (on) cell.appendChild(el(`<div class="tag">${L('equipped')}</div>`));
      ...
      const b = el(`<button class="btn gem" style="padding:4px 10px;font-size:11px">💎 ${price}</button>`);
      b.onclick = () => {
        if (st.gems >= price) { st.gems -= price; st.skins.push(s.id); SFX.fanfare(); renderPanel(); refreshDots(); writeSave(); }
        else floatToast('💎 ' + L('need') + ' ' + fmt(price - st.gems));
      };
      cell.appendChild(b);
    }
...
function renderTasks(body) {
  ...
  for (const t of st.tasks.list || []) {
    ...
    const row = el(`<div class="row ${done ? 'task-done' : ''}">
      ...
      <button class="btn ${done && !t.claimed ? 'green' : 'gray'}" ${done && !t.claimed ? '' : 'disabled'}>💎 ${t.gem}</button></div>`);
    row.querySelector('button').onclick = () => {
      if (t.prog >= t.goal && !t.claimed) {
        t.claimed = true; st.gems += t.gem;
        SFX.gem(); floatToast(`+${t.gem} 💎`);
        renderPanel(); refreshDots(); writeSave();
      }
    };
    body.appendChild(row);
...
function renderPrestige(body) {
  ...
  body.appendChild(el(`<div class="core-big">🌀 ${st.cores}</div>`));
  ...
  if (!pi.labOpen) {
    ...
    body.appendChild(el(`<div class="row"><div class="grow"><h4>🌀 ${L('prestige')}</h4>
    ...
    const row = el(`<div class="row"><div class="grow"><h4>🌀 ${L('prestige')}</h4><div class="sub">${L('cores')} +${pi.cores} · ${zh ? '已跃迁' : 'dives'} ×${st.prestiges}</div></div>
    ...
    for (const l of LAB) {
      ...
      const cell = el(`<div class="lab-cell"><h5>${zh ? l.nZ : l.nE} <span style="color:var(--tx2)">${lv}/${l.max}</span></h5>
        ...
        <button class="btn ${can ? 'gem' : 'gray'}" style="margin-top:6px;padding:4px 10px;font-size:11px" ${can ? '' : 'disabled'}>🌀 ${maxed ? L('maxed') : cost}</button></div>`);
...
function openPrestigeConfirm() {
  const zh = getLang() === 'zh';
  const pi = meta.prestigeInfo();
  showModal(`<h3>🌀 ${L('prestige')}</h3>
    <div class="m-sub">${L('prestigeConfirm')}</div>
    <div class="big-reward">+${pi.cores} 🌀</div>
...
export function showModal(html, setup) {
  closeModal();
  modalEl = el(`<div class="modal-mask"><div class="modal">${html}</div></div>`);
  ...
  showModal(`<h3>⚓ ${L('offlineTitle')}</h3>
    ...
    m => {
      m.querySelector('#of-x2').onclick = async () => {
        if (await requestAd('offline_x2')) { addCoins(res.coins * 2); floatToast(`+${fmt(res.coins * 2)} 🪙`); SFX.coin(4); closeModal(); refreshDots(); writeSave(); }
      };
      m.querySelector('#of-col').onclick = () => { addCoins(res.coins); floatToast(`+${fmt(res.coins)} 🪙`); SFX.coin(2); closeModal(); refreshDots(); writeSave(); };
    });
}
...
function drawWheel(cv, rot) {
  ...
  WHEEL.forEach((s, i) => {
    ...
    c.translate(R * .68, 0); c.rotate(Math.PI / 2);
    c.font = '44px sans-serif'; c.textAlign = 'center'; c.textBaseline = 'middle';
    c.fillText(s.em, 0, 0);
    c.restore();
  });
  c.restore();
...
function doSpin(useAd, mm) {
  ...
  let lastTick = 0;
  // time-based interval loop: completes even in throttled/occluded webviews
...
export function openGacha() {
  const zh = getLang() === 'zh';
  const g = meta.gachaState();
  showModal(`<h3>🥚 ${L('gacha')}</h3>
    <div class="m-sub">${L('pityNote')} · ${zh ? '已有蛋票' : 'tickets'}: ${g.tickets} · 💎 ${g.gems}</div>
    <div class="btns">
      <button class="btn gem" id="ga-1">${L('open1')}</button>
      <button class="btn gem" id="ga-10">${L('open10')}</button>
      ${g.tickets > 0 ? `<button class="btn green" id="ga-t">🎟️ ${zh ? '用蛋票开 1 个' : 'Open 1 with ticket'}</button>` : ''}
...
function pull(n, ticket, m) {
  ...
  if (!res) { floatToast('💎 ' + L('need')); return; }
  SFX.pop();
  const zh = getLang() === 'zh';
  const cards = res.map((r, i) => {
    const em = r.type === 'skin' ? '🐾' : r.type === 'gems' ? '💎' : r.type === 'buff' ? '⚡' : r.type === 'coins' ? '🪙' : r.type === 'dup' ? '🔁' : '❓';
  ...
  showModal(`<h3>🥚 ${L('gacha')}</h3>
    ...
    <div class="note">${zh ? '重复皮肤自动转换为 40 宝石' : 'Duplicate skins convert to 40 gems'}</div>
    ...
    mm => {
      mm.querySelector('#ga-again').onclick = () => { meta.gachaPull(0, false); openGacha(); };
...
export function openResearch() {
  ...
  showModal(`<h3>🔬 ${L('research')}</h3><div class="m-sub">${zh ? '选择一项研究，完成后获得限时增益' : 'Pick one research; finish it for a timed buff'}</div>
    <div class="btns">${opts.map((o, i) => `<button class="btn ${i ? 'gem' : 'gold'}" data-r="${o.id}">${BUFFS[o.buff].em} ${zh ? o.nZ : o.nE} · ${o.min}min (${Math.ceil(o.time / 60)}min)</button>`).join('')}
...
export function openSignin() {
  ...
  const cells = SIGNIN.map((rw, i) => {
    ...
    return `<div class="sign-cell ${done ? 'done' : ''} ${today ? 'today' : ''} ${i === 6 ? 'big' : ''}">
      <div class="d">${L('day')} ${i + 1}</div><div class="em">${rw.em}</div>
      <div class="d">${rw.kind === 'coins' ? '🪙' + rw.min + 'm' : rw.kind === 'gems' ? rw.n : rw.kind === 'buff' ? rw.min + 'min' : rw.kind === 'egg' ? '×' + rw.n : 'BIG'}</div></div>`;
  }).join('');
  showModal(`<h3>📅 ${L('signin')}</h3>
  ...
  showModal(`<h3>⚙ ${L('settings')}</h3>
    ...
    m => {
      ...
      m.querySelector('#se-reset').onclick = () => {
        showModal(`<h3>⚠</h3><div class="m-sub">${L('resetConfirm')}</div>
...
// ---------- toast ----------
export function floatToast(text) {
  const layer = document.getElementById('fx-layer');
  if (!layer) return;
  const el2 = document.createElement('div');
  el2.className = 'ftext';
  el2.textContent = text;
  el2.style.cssText = `left:50%;top:38%;transform:translateX(-50%);font-size:20px;color:#ffd257;`;
